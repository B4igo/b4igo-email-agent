...
|---|---|---|---|
| `userId` | String | Yes | Maps from `username` |
| `typeId` | Int | Yes | Doctor type ID - **TODO: resolve via `getTypeOfDoctor` API; currently hardcoded 0** |
| `typeName` | String | Yes | e.g. "General", "Cardiologist". Maps from `Doctor.type` |
| `doctorName` | String | Yes | Maps from `Doctor.doctor_name` |
| `contactInformation` | String | Yes | Maps from `Doctor.location` |
| `city` | String | Yes | Maps from `Doctor.location` (same field, no separate city) |
| `stateName` | String | Yes | Not in schema - sent as `""` |
| `countryName` | String | Yes | Not in schema - sent as `""` |
| `createdBy` | String | Yes | Same as `userId` |
| `markAsImportant` | Boolean | Yes | Hardcoded `false` |
...
|---|---|---|---|
| `userId` | String | Yes | |
| `doctorId` | Int | Yes | `_RECORD_ID_FIELD["doctor"] = "doctorId"` |
| `doctorName` | String | Yes | Maps from `Doctor.doctor_name` |
...
|---|---|---|---|
| `userId` | String | Yes | Maps from `username` |
| `memberName` | String | Yes | Hardcoded to `username` (no member name in schema) |
| `memberId` | String | Yes | Not in schema - sent as `""` |
| `groupId` | String | Yes | Not in schema - sent as `""` |
...
|---|---|---|---|
| `userId` | String | Yes | |
| `insuranceId` | Int | Yes | `_RECORD_ID_FIELD["insurance"] = "insuranceId"` |
...
|---|---|---|---|
| `userId` | String | Yes | Maps from `username` |
| `medicine_name` | String | Yes | Maps from `Medication.name_of_medicine` |
| `treatment_name` | Int | Yes | Not in schema - **currently omitted (TODO)** |
| `purpose` | String | Yes | Maps from `Medication.purpose` |
| `dosage` | String | Yes | Not in schema - **currently omitted (TODO)** |
| `start_date` | String | Yes | Maps from `Medication.date` |
| `end_date` | String | Yes | Not in schema - **currently omitted (TODO)** |
| `frequency_dosage` | String | Yes | Not in schema - **currently omitted (TODO)** |
| `side_effect` | String | Yes | Not in schema - **currently omitted (TODO)** |
| `markAsImportant` | Boolean | Yes | Hardcoded `false` |
| `files` | [File] | Yes | Not used - **currently omitted** |
...
|---|---|---|---|
| `userId` | String | Yes | |
| `recordId` | Int | Yes | `_RECORD_ID_FIELD["medication"] = "recordId"` |
...
> Warning: **Discrepancy in API.md:** Two sections conflict.
...
|---|---|---|---|
| `userId` | String | Yes | Maps from `username` |
| `history` | String | Yes | Combined `"{disease} - {description}"` or just `disease` |
...
|---|---|---|---|
| `userId` | String | Yes | |
| `historyId` | Int | Yes | `_RECORD_ID_FIELD["medical_history"] = "historyId"` |
